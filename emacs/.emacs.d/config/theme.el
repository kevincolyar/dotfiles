;;; theme --- Light/dark theme, following the client terminal  -*- lexical-binding: t; -*-
;;; Commentary:
;; Loaded after ui.el: doom-modeline has to exist before the first theme load,
;; so `my/refresh-modeline' can rebuild its bars.
;;; Code:
...
;; `transient' and `markdown-mode' faces only exist once their package is
;; loaded, so re-run the tweaks then as well.
